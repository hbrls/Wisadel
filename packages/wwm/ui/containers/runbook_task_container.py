...
class TaskSoloEpisode(CardWidget):
    """Task 专用的 FileSelector + SoloRunner 横向排列执行单元
    ...
    def start_run(self):
        ...
        print(f"[TaskSoloEpisode] run: cwd={cwd}, prompt={prompt}")
    ...
    def _on_worker_error(self, message: str):
        print(f"[TaskSoloEpisode] error: {message}")
...
class RunbookTaskContainer(QWidget):
    ...
    def _setup_ui(self):
        ...
        for episode_data in self.task.episodes:
            episode = TaskSoloEpisode(parent=self)
    ...
    def _sync_ui(self):
        ...
        for i, episode in enumerate(self._episodes):
            episode_data = self.task.episodes[i]
            episode.set_working_directory(self.task.working_directory)
    ...
    def _on_directory_changed(self, path: str):
        ...
        self._sync_ui()
        print(f"[TaskContainer] working_directory={self.task.working_directory}")

    def _on_file_changed(self, episode_index: int, relative_path: str):
        if self._syncing:
            return
        self.task.episodes[episode_index].filename = relative_path
        self._sync_ui()
        print(f"[TaskContainer] episode[{episode_index}].filename={relative_path}")
    ...
    def _on_play_clicked(self):
        ...
        if action in ("start_run", "continue_run"):
            ...
            self._run_button.setEnabled(False)
            episode.start_run()
            print(f"[TaskContainer] Run started (action={action}, phase={self.task.state_machine.current_phase})")
        else:
            print("[TaskContainer] Stop - all episodes completed")
    ...
    def _on_episode_finished(self):
        ...
        if action == "continue_run":
            ...
            episode = self._episodes[episode_index]
            episode.start_run()
            print(f"[TaskContainer] Run completed, continuing... (phase={self.task.state_machine.current_phase})")
        else:
            self._run_button.setEnabled(True)
            ...
            print(f"[TaskContainer] Run completed, all episodes finished (phase={self.task.state_machine.current_phase})")
        self._sync_ui()